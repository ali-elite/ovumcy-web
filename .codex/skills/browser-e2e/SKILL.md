...
3. Generate or update Playwright tests (when needed)
   ...
   - When dashboard or calendar day surfaces gain a second HTMX write form near the main day-save form (for example a manual cycle-start action), target the journal save form via `data-save-feedback` or another dedicated hook and add a separate scenario for the new action instead of reusing broad `/api/days/` form selectors.
   - When owner day notes are behind a disclosure, update shared Playwright helpers to open the visible summary first and keep at least one scenario that proves the disclosure stays closed for empty notes but reopens after a saved note exists.
   - For PWA install UI validation or demo captures, prefer a synthetic `beforeinstallprompt` event on a local instance instead of relying on device-specific native prompts or a public demo environment.
   ...
   - For auth and recovery UX, assert custom validation outcomes (`novalidate`, client status blocks, stale server-error clearing) and disabled confirmation CTAs instead of relying on native browser tooltips or attempting to click disabled buttons.
   - For localized segmented date fields, prefer assertions against user-visible outcomes such as preserved values and disabled actions instead of native `validationMessage` on the hidden transport input.
   - When a page uses a fixed bottom tabbar, add one focused mobile-viewport check that a lower interactive control remains clickable without force-clicking. Prefer this over speculative spacing tweaks when exploratory monkey clicks are the only signal.
   - For fixed bottom navigation on mobile, prefer one geometry-based regression that checks a lower control or legend sits above the tabbar after scrolling, instead of relying only on screenshots or forced clicks.
   - For visual overflow regressions that are hard to assert pixel-perfect, prefer a narrow structural contract (for example an `overflow-hidden` wrapper or `aria-expanded` state) plus one user-visible behavior assertion.
...
- Step 1 quick-picks in a non-English locale → verify relative labels stay localized and do not fall back to English  
...
- With "Auto-fill period days" disabled, complete onboarding → verify the baseline range affects predictions only and does not create logged-entry markers  
...
- For confirmation-gated dashboard actions, include both accept and cancel paths in the scenario. Cancel must be side-effect free and must not leave shared `status-error` markup behind.  
...
- When onboarding can preseed cycle data, distinguish predicted baseline states from real logged-entry markers. A disabled auto-fill toggle must keep `data-calendar-has-data=false` even when the same date range is still used for cycle predictions.  
