...
This README describes the current `main` branch. The latest tagged release is `v0.6.1`.
...
Uses the prebuilt image from GHCR pinned to the latest tagged release by default (`ghcr.io/terraincognita07/ovumcy:v0.6.1`).
...
```bash
OVUMCY_IMAGE=ghcr.io/terraincognita07/ovumcy:v0.6.1 docker compose up -d
```
