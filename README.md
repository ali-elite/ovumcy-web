...
This README describes the current `main` branch. The latest tagged release is `v0.8.1`.
...
Uses the prebuilt image from GHCR pinned to the latest tagged release by default (`ghcr.io/ovumcy/ovumcy-web:v0.8.1`).
...
```bash
OVUMCY_IMAGE=ghcr.io/ovumcy/ovumcy-web:v0.8.1 docker compose up -d
```
